...
var addClassNameListener = function addClassNameListener(elemClass, callback) {
...
var EasyCart = function (module, $) {
  ...
  hasRegisteredHook = function hasRegisteredHook(name, callback) {
    if (typeof hooks[name] !== 'undefined') return true;
    return false;
  };

  callHook = function callHook(name, args) {
    // console.log("callHook", name, args);
    if (typeof hooks[name] !== 'undefined') for (var i = 0; i < hooks[name].length; ++i) {
...
ShopifyAPI.updateCart = function (cartUpdates, callback) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var quantity = parseInt(cartUpdates[id]);
    var timer = 1000;
...
var likingBadgeTexts = {
  ...
  variant2: '<strong>{{ name }}</strong> und <strong>{{ likes }}</strong> andere lieben dieses Produkt'
}; // prettier-ignore
...
function renderLikingBadges() {
  ...
  var likingBadgeElements = document.querySelectorAll('.liking-badge');
  if (likingBadgeElements.length === 0) return;
  var _window = window,
      productId = _window.productId;
  var likesCount = getLocalStorage("likes-".concat(productId));

  if (!likesCount) {
    likesCount = getRandomNumberBetween(380, 1537).toString();
    setLocalStorage("likes-".concat(productId), likesCount);
  }

  var text = likingBadgeTexts[window.drip.testVariant];
  text = text.replace('{{ name }}', 'Nina Moghadamm');
  text = text.replace('{{ likes }}', likesCount);
  likingBadgeElements.forEach(function (badgeElement) {
    var textElement = badgeElement.querySelector('.liking-badge__text');
